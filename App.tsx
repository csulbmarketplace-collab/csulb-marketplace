...
const now = () => Date.now();
const uuid = () => "id_" + Math.random().toString(36).slice(2);
...
export default function App() {
  ...
  const [items, setItems] = useState<any[]>([]); // fresh page
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter(i => {
        if (tab === "auctions") return i.type === "auction";
        if (tab === "buy") return i.type === "fixed";
        return true;
      })
  ...
  return (
    <div className="app-root">
      <TopBar user={user} onLogout={()=>setUser(null)} search={search} setSearch={setSearch} cat={cat} setCat={setCat} onHome={()=>setMode("home")} onExplore={()=>setMode("market")} onSignIn={()=>setMode("market")} />
      {mode === "home" ? (
