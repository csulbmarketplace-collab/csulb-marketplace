...
function AuthGate({
  ...
  // simple in-memory demo accounts; swap for real auth later
  ...
  return (
    <section className="section authWrap">
      {/* Back to Home (kept subtle, above the card) */}
      ...
      <div className="glass authCard">
        ...
        {error && (
          <div style={{ color: "#ffb4b4", marginTop: 10, fontWeight: 700 }}>
            {error}
          </div>
        )}
...
function HomeLanding({
  ...
  return (
    <>
      {/* Top bar with breathing room + brand */}
      ...
      <section className="section">
        <div className="container">
          <div className="glass" style={{ padding: "clamp(18px,4vw,36px)" }}>
            <h1 className="h1">
              Buy & sell on campus — safely, fast, and student-only
            </h1>
            <p className="lead">
              Auctions and buy-now listings, verified with{" "}
              <strong>@csulb.edu</strong>.
            </p>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <button className="btn-primary btn-lg" onClick={onSignIn}>
                Sign in
              </button>
              <button className="btn btn-lg" onClick={onExplore}>
                Explore marketplace
              </button>
            </div>
          </div>
          ...
          <div className="stickyCta">
            <div className="stickyBar">
              <button className="btn-primary btn-block" onClick={onSignIn}>
                Sign in
              </button>
  ...
  if (mode === "home" && !user) {
    return (
      <HomeLanding
        ...
        onExplore={() => alert("Browse as guest (demo)")}
      />
    );
  }
  if (!user) {
    return <AuthGate onLogin={(u) => setUser(u)} onBack={() => setMode("home")} />;
  }
  return (
    <section className="section">
      <div className="container">
        <div className="glass" style={{ padding: 24 }}>
          ...
          <p className="lead" style={{ marginTop: 8 }}>
            You’re logged in. Marketplace feed coming next.
          </p>
          <button className="btn mt-16" onClick={() => location.reload()}>
            Log out
          </button>
        </div>
      </div>
